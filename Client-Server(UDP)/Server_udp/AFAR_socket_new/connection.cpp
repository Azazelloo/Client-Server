...
int connectionServer(DEVICE* obj_dev)//const char* adress, int& port, SOCKET* my_sock)
{
	...
	cout << "ќжидание соединени€..." << endl;
	WSADATA ws;
	sockaddr_in addr;
	int ret_val;

	if (FAILED(WSAStartup(MAKEWORD(2,2), &ws))) //инициализаци€ Winsock
	{
	...
	stop
	if (serverSock==INVALID_SOCKET)//провер€ем создание сокета
	{
	...
	SOCKADDR_IN sin; //структура описани€ сокета
	...
	printf("ѕрин€то соединение от %s, порт %d\n", inet_ntoa(ip_address), obj_dev->port);
